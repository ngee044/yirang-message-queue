...
namespace DataBase
{
	...
	public:
		...
		// Adds a column to an existing table if it is not present yet. `CREATE TABLE IF NOT EXISTS`
		// never alters a table that already exists, so a schema file alone cannot upgrade a
		// database created by an earlier version. No-op when the column is already there.
